...
const getLoggedInUser = asyncHandler(async (req, res) => {
  const user = await Users.findOne(
    { _id: req.user._id },
    { updatedAt: 0, verified: 0, __v: 0, password: 0, pText: 0 }
  )
  const { is_admin, suspended, ...rest } = user?._doc
  res.status(200).json({ ...rest, isAdmin: is_admin, suspended })
})

const getUserMetrics = asyncHandler(async (req, res) => {
  // Get withdrawals ,
  let withdrawals = await Transactions.find({
    author: req.user._id,
    type: 'withdrawal',
    status: 'approved',
  })
  let deposits = await Transactions.find({
    author: req.user._id,
    type: 'deposit',
    status: 'approved',
  })
...
const updateUserProfileImage = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json({ success: true, url, message: 'Profile image updated successfully' })
})
...
const getAdminMetrics = asyncHandler(async (req, res) => {
  ...
  const totalDeposits = deposits.reduce(
    (totalDeposits, deposit) =>
      Math.round((totalDeposits + deposit.amount) * 100) / 100,
    0
  )
  ...
  const totalWithdrawals = withdrawals.reduce(
    (totalWithdrawals, withdrawal) =>
      Math.round((totalWithdrawals + withdrawal.amount) * 100) / 100,
    0
  )

  let users = await Users.find({}, { profit: 1 })
  let profits = users.reduce(
    (totalProfits, user) =>
      Math.round((totalProfits + user.profit) * 100) / 100,
    0
  )
...
const addProfitToUser = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json({ success: true, message: 'Profit successfully added to account' })
})
...
const suspendUser = asyncHandler(async (req, res) => {
  ...
  await Users.updateOne({ _id: userId }, { $set: { suspended: true } })
...
const unsuspendUser = asyncHandler(async (req, res) => {
  ...
  await Users.updateOne({ _id: userId }, { $set: { suspended: false } })
