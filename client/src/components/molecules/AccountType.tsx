...
const AccountType = ({
  ...
  const [value, setValue] = useState(Math.floor(min))
  const user = useSelector(useUserSlice)
  const multipliedValue = value * 1.99
    ...
    <div className="flex flex-col w-full rounded-2xl overflow-clip">
      ...
      <div className="flex flex-col gap-3 p-[20px] bg-[#191f31] ">
        ...
        <input
          ...
          min={0}
          max={max}
          className="range pr-6 accent-sec h-12"
        ...
        {!user.isAdmin && (
          <PrimaryButton
            text={!user?._id ? 'Sign up' : 'Trade'}
            href={!user?._id ? '/signup' : '/dashboard/trades/new'}
            sx="rounded-[5px] bg-[#52AFEE] !py-3"
          />
        )}
      </div>
    </div>
