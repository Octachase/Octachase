        ...
        <nav className="w-full lg:w-auto flex flex-col lg:flex-row  lg:p-0  lg:relative">
          <ul className="flex flex-col lg:flex-row  lg:items-center gap-3 text-[12px] font-medium">
            <li>
              <Link
                href="/"
                className={`block p-[8px] mb-4 lg:mb-0 ${
                  pathname === '/' ? 'text-sec' : 'hover:text-sec'
                } font-bold`}
              >
                HOME
              </Link>
            ...
            <li>
              <Link
                href="/about"
                className={`block p-[8px] mb-4 lg:mb-0 ${
                  pathname === '/about' ? 'text-sec' : 'hover:text-sec'
                } font-bold`}
              >
                ABOUT US
              </Link>
            ...
            <li>
              <Link
                href="/faq"
                className={`block p-[8px] mb-4 lg:mb-0 ${
                  pathname === '/faq' ? 'text-sec' : 'hover:text-sec'
                } font-bold`}
              >
                FAQS
              </Link>
            ...
            <li>
              <Link
                href="/contacts"
                className={`block p-[8px] mb-4 lg:mb-0 ${
                  pathname === '/contacts' ? 'text-sec' : 'hover:text-sec'
                } font-bold`}
              >
                CONTACTS
              </Link>
        ...
        <div className="w-full flex  items-center justify-between lg:w-auto mt-6 lg:mt-0">
          ...
          {!user?._id && (
            <div className="w-full flex flex-col lg:flex-row items-center gap-4 lg:gap-[10px]">
              <Link
                href="/login"
                className="w-full lg:w-auto bg-transparent hover:bg-opacity-70 border-[1px] border-sec text-[15px] px-[20px] py-4 lg:py-[8px] font-bold rounded-md"
              >
                Login
              </Link>
              <Link
                href="/signup"
                className="w-full lg:w-auto py-4 bg-sec hover:bg-opacity-70  text-sm font-bold px-[20px] lg:py-[10px] rounded-md"
              >
                Signup
              </Link>
            </div>
