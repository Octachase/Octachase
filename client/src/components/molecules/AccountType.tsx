	...
	return (
		<div className="flex flex-col w-full rounded-2xl overflow-clip">
			...
			<div className="flex flex-col gap-3 p-[20px] bg-[#191f31] ">
				...
				<p className="text-[14px] font-bold text-center ">${formatNumberIntoMoney(value)}</p>
				<input type="range" name="amount" value={value} id="amount" min={0} max={max} className="range pr-6 accent-sec h-12" onChange={(e) => setValue(+e.target.value)} />
