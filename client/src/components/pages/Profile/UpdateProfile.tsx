...
const UpdateProfile = () => {
  ...
  const { register, reset, handleSubmit, getValues } =
    useSelectedPropertiesFromHookForm(updateProfileSchema)
      ...
      <form className="mt-6" onSubmit={handleSubmit(updateUser)}>
        <div className="w-full mb-3 ">
          ...
          <input
            type="text"
            {...register('firstname')}
            className="bg-[#464551] block border-[1px] focus:outline-0 px-2 w-full py-2 text-sm"
            id="firstname"
          />
        </div>
        <div className="w-full mb-3 ">
        ...
        <div className="w-full mb-3 ">
          <p className="text-[15px] rounded-[5px] mb-[2px] block">
            Email Address
          </p>
          <p
            className="block text-sm bg-slate-600  text-white border-[1px] focus:outline-0 px-2 w-full py-2 "
            id="fullname"
          >
            {user?.email}
          </p>
