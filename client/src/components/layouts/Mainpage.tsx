  ...
  return (
    <div className="w-full h-auto relative flex  bg-[#4A4445] items-stretch justify-start">
      ...
      <div
        ...
        } bg-[#4A4445] md:w-1/2 z-[5] lg:w-1/5 h-screen lg:sticky top-0 left-0 border-r-[1px]`}
      >
        <div className="h-16 border-b-[1px] lg:px-3 flex items-center  px-4 justify-between">
          <Link href="/" className="w-28 h-1/2 relative">
            <Image
              src="/public/assets/dextachase-logo.png"
              fill
              alt="Logo Image"
            />
          </Link>
          <button
            className="group lg:hidden hover:bg-slate-800 p-[5px] rounded-full"
        ...
        <div className="w-full p-4 h-[90vh] flex flex-col justify-between">
          ...
          <button
            className={`flex items-center gap-3 mb-2 hover:bg-[#5A5557] opacity-50 text-sm hover:opacity-80 rounded-[5px] py-3 p-2`}
            onClick={() => logoutUser()}
          >
      ...
      <div className="w-full lg:w-4/5 min-h-screen relative">
        ...
        <div className="w-full h-16 border-b-[1px] flex px-4 lg:px-6 items-center justify-center py-4 bg-[#4A4445] shadow-md sticky top-0 left-0 z-[3]">
          <div className="flex items-center justify-center gap-[3px]">
            <button
              className="w-10 h-10 lg:hidden group rounded-full hover:bg-slate-800 flex items-center justify-center ml-auto"
              onClick={() => setShowMenu(true)}
            >
              <LuMenu className="group-hover:text-black text-xl" />
            </button>
            {user?.isAdmin && isAdmin && (
              <p className="uppercase font-bold">Admin Section</p>
            )}
          </div>
          <button
            className="w-10 h-10 group rounded-full hover:bg-slate-800 flex items-center justify-center ml-auto"
      ...
      {showInformation && (
        <AccountInformation setShowInformation={setShowInformation} />
      )}
    </div>
  )
}
