...
const { sendResetPasswordEmail, sendVerifyEmail } = require("../../libs/nodemailer");

const controllerLoginUser = asyncHandler(async (req, res) => {
	const { email, password } = req.body;

	const user = await Users.findOne({ email }, { __v: 0, createdAt: 0, updatedAt: 0 });
	...
	if (!user.verified) {
		...
		res.status(403);
		throw new Error("The account is not verified. Please check your email to verify your account");
	}
	...
	const { password: userPassword, verified, is_admin, ...rest } = user?._doc;

	res.status(200).json({ user: { ...rest, isAdmin: is_admin }, token });
});
const controllerRegisterUser = asyncHandler(async (req, res) => {
	...
	// Create new user
	let user = new Users({ email, password, firstname, lastname, verified: false });
	...
	res.status(200).json({ success: true, message: "Registration was successful. Please check email to verify your account" });
});
...
const verifyAccount = asyncHandler(async (req, res) => {
	...
	await Users.updateOne({ email: check.email }, { $set: { verified: true }, $unset: { code: null } });
...
const controllerResetPassword = asyncHandler(async (req, res) => {
	...
	// Send reset email
	await sendResetPasswordEmail({ email: user.email, link, username: user.firstname });

	res.status(200).json({ success: true, message: "An email has been sent. Check and proceed with reset" });
});
...
const controllerSetPassword = asyncHandler(async (req, res) => {
	...
	if (!user.verified) {
		// Check code		if (!valid) {
		res.status(401);
		throw new Error("You cannot reset password for an unverified account. Please contact support for more info");
	}
	...
	await user.save();
	res.status(200).json({ success: true, message: "Password has been reset successfully. Proceed to login" });
});
...
module.exports = { controllerRegisterUser, controllerResetPassword, controllerSetPassword, controllerLoginUser, changeUserPassword, verifyAccount, controllerDeleteAccount };
