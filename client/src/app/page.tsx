			...
			<section className="w-full h-[75vh] md:h-[60vh] lg:h-auto lg:pt-24 bg-[url('/assets/bg-about-header.jpg')] bg-cover px-4">
				<div className="flex relative overflow-hidden items-center w-full max-w-6xl mx-auto h-full">
					...
					<div className="flex flex-col gap-[20px] w-[470px] h-auto -mt-24 lg:mt-0 lg:h-[600px]">
						<h1 className="text-2xl md:text-[40px] font-medium leading-[35px] md:leading-[50px]">RELIABLE , SIMPLE AND INNOVATIVE</h1>
						...
						<PrimaryButton
							...
							href={!user?._id ? "/login" : user.isAdmin ? "/admin" : "/dashboard"}
							sx="!bg-[#52afee] rounded-[10px] w-full md:w-96"
						/>
						{!user?._id && <PrimaryButton text={"Sign up"} href={"/signup"} sx="!bg-transparent border-[1px] rounded-[10px] w-full md:w-96" />}
					</div>
				</div>
			</section>
			{/* commodities chart */}
				...
				<div className="flex flex-col max-w-6xl px-4 lg:px-24 mx-auto">
					...
					<article className="w-full flex flex-col md:flex-row gap-12 mb-10">
						<p className="basis-[50%] text-[14px]">
							...
							weekends. Octachase customer service is available 24/7. We are continuously adding new financial instruments.
						</p>
					...
					{!user?.isAdmin && (
						<PrimaryButton text={"Setup your trading account"} href={!user?._id ? "/login" : "/dashboard/trades"} sx="w-full !bg-[#52afee] mx-auto rounded-[10px] lg:w-96" />
					)}
				...
				<div className="flex flex-col lg:flex-row gap-12 md:gap-[20px] mt-[50px]">
					...
					<div className="flex flex-col w-full -mt-6 lg:mt-0 gap-[30px]">
						<p className="text-[14px] leading-9 mt-3">
							Octachase allows you to actively trade most popular cryptocurrencies such as Bitcoin, Ethereum, Ripple, Litecoin and more, profit from market rallies and declines, or hedge
						...
						{!user?.isAdmin && <PrimaryButton text={"Setup your trading account"} href={!user?._id ? "/login" : "/trades"} sx="!bg-[#52afee] rounded-[10px] w-72" />}
					</div>
